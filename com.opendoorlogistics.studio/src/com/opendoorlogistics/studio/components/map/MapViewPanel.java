...
//	private final MapObjectsRenderer renderer;
	...
	public MapViewPanel(MapApi mapi){
		...
		BackgroundTileFactorySingleton.getFactory().addTileListener(this);
		...
		for (int x = 0; x <= numWide; x++) {
			for (int y = 0; y <= numHigh; y++) {
				...
				if (g.getClipBounds().intersects(new Rectangle(itpx * tileSize - viewportBounds.x, itpy * tileSize - viewportBounds.y, tileSize, tileSize))) {
					Tile tile = factory.getTile(itpx, itpy, zoomLevel);
					...
					if (itpx< 0 || itpy < 0 || itpx>mapSize.width || itpy > mapSize.height) {
						...
						Tile superTile = factory.getTile(itpx / 2, itpy / 2, zoomLevel + 1);

						if (superTile.isLoaded()) {
							int offX = (itpx % 2) * tileSize / 2;
							int offY = (itpy % 2) * tileSize / 2;
							g.drawImage(superTile.getImage(), ox, oy, ox + tileSize, oy + tileSize, offX, offY, offX + tileSize / 2, offY + tileSize / 2, null);
						} else {
							g.setColor(Color.GRAY);
							g.fillRect(ox, oy, tileSize, tileSize);
						}
	...
	@Override
	public void dispose() {
		BackgroundTileFactorySingleton.getFactory().removeTileListener(this);
	}
