...
public class InteractiveMapPanel extends ReadOnlyMapPanel implements MapSelectionList {
			...
			if (newProportion != -1) {
				...
				if (space > 0) {
					...
					if (getOrientation() == JSplitPane.HORIZONTAL_SPLIT) {
						...
						// get current geographic position at what would be the new on-screen centre
						LatLong pos =converter.getLongLat((int) Math.round(interactive.getWidth() / 2.0),
								(int) Math.round(newMapHeight / 2.0) - 2);
	...
	public InteractiveMapPanel(MapConfig config,List<? extends DrawableObject> pnts, final ODLDatastoreUndoable<ODLTableAlterable> globalDs, GlobalMapSelectedRowsManager gsm) {
		...
		interactive.addOnFillListener(new OnFillListener() {
			...
			public void onFill(TLongArrayList selected) {
				String[] field = fieldSelectorPanel.getField();
				String value = fieldSelectorPanel.getFillValue();

				// find table...
				ODLTable table = null;
				int col = -1;
				if (field != null && field.length == 2 && field[0] != null && field[1] != null) {
					table = TableUtils.findTable(globalDs, field[0], true);
					if (table != null) {
						col = TableUtils.findColumnIndx(table, field[1], true);
					}
				}
				...
				for (long id : selected.toArray()) {
					...
					if (objTable == null || objTable.getImmutableId() != table.getImmutableId()) {
						continue;
					}

					table.setValueById(value,id, col);
					nbSet++;
				...
				if (nbProcessed > 0 && nbSet == 0) {
					JOptionPane.showMessageDialog(InteractiveMapPanel.this,
							"No values were set as none of the selected objects were from the selected table \"" + field[0] + "\".");
				}
			}
		});
	...
	private class FillFieldPanel extends JPanel implements ODLListener {
		...
		FillFieldPanel(ODLDatastore<? extends ODLTable> globalDs) {
			...
			text = new JTextField();
			add(LayoutUtils.createVerticalBoxLayout(new JLabel("Set fill value:"), text), BorderLayout.SOUTH);
		}
