/*******************************************************************************
 * Copyright (c) 2014 Open Door Logistics (www.opendoorlogistics.com)
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the GNU Lesser Public License v3
 * which accompanies this distribution, and is available at http://www.gnu.org/licenses/lgpl.txt
 ******************************************************************************/
package com.opendoorlogistics.components.geocode.postcodes;

import java.io.Serializable;

import javax.xml.bind.annotation.XmlAttribute;
import javax.xml.bind.annotation.XmlRootElement;

@XmlRootElement
public class PCDatabaseSelectionConfig implements Serializable{
	/**
	 * 
	 */
	private static final long serialVersionUID = 5223751199023472034L;
	String geocoderDbFilename="";

	public PCDatabaseSelectionConfig(String geocoderDbFilename) {
		this.geocoderDbFilename = geocoderDbFilename;
	}
	
	public PCDatabaseSelectionConfig() {
		
	}

	public String getGeocoderDbFilename() {
		return geocoderDbFilename;
	}

	@XmlAttribute
	public void setGeocoderDbFilename(String geocoderDbFilename) {
		this.geocoderDbFilename = geocoderDbFilename;
	}
	
}
